...
@router.delete("/realms/{realm_name}", status_code=204)
async def delete_realm(realm_name: str):
    """Delete a Keycloak realm by name."""
    if realm_name == "master":
        raise HTTPException(status_code=400, detail="Cannot delete the 'master' realm.")
